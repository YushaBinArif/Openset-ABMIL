...
def predict_bag_with_modified_attention(bag_tensor, ins_model, ocsvm_model, bag_classifier):
    ins_model.eval()
    with torch.no_grad():
        ...
        logits_np = logits.cpu().numpy()  # Convert to numpy

        # 2. Use One-Class SVM to detect negative class
        predictions = ocsvm_model.predict(logits_np)  # 1 = inlier (negative), -1 = outlier
        ...
        # 4. Weighted feature aggregation
        z = torch.mm(attention_tensor, features)  # (1 x feature_dim)

        # 5. Classify the bag
        class_prob = bag_classifier(z)
        return class_prob, None, z  # unk_prob is None (since OpenMax not used here)
...
if __name__ == '__main__':
    ...
    # Train ocsvm using instance-level logits from class 0
    ocsvm_model = train_ocsvm(features, labels)
