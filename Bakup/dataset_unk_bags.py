...
class bagDataset(torch.utils.data.Dataset): # dataloader with bag labels
    def __init__(self, num_bag=1000, bag_size=10, train = True, transform = None, epoch=0):
        ...
        #Unknown class handling
        if not train:
            for _ in range(self.num_bag):  # create unknown bags
                num_uins = np.random.randint(1, self.bag_size + 1)
                np.random.shuffle(u_data)
                np.random.shuffle(n_data)
                
                # Start with unknown instances
                instances = u_data[0].reshape(ins_dim)
                for i in range(num_uins - 1):
                    tmp = u_data[i+1].reshape(ins_dim)
                    instances = np.vstack([instances, tmp])
                
                # Fill rest of the bag with negatives
                for i in range(self.bag_size - num_uins):
                    tmp = n_data[i].reshape(ins_dim)
                    instances = np.vstack([instances, tmp])
                
                self.bags.append(torch.from_numpy(instances).float())
                self.labels.append(torch.tensor([3]).long())  # class_u
